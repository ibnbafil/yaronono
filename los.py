...
while running:
    ...
    for obs in obstacles[:]:
        ...
        if score == 99:
            obstacle_width = 750  # Толщина 100-го препятствия в 5 раз больше обычного
        else:
        ...
        if (plane_x + 80 > obs[0] and plane_x < obs[0] + obstacle_width) and \
           (plane_y < obs[1] or plane_y + 40 > obs[1] + obstacle_gap):
            if score != 99:  # Проигрыш только для обычных препятствий, не для особого
    ...
    if score == 99 and plane_y + 40 >= HEIGHT and abs(speed) < 318:
        display_win()
